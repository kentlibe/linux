...
static int lt8722_reg_read(struct spi_device *spi, u8 reg, u32 *val)
{
	...
	if (rx_buf[LT8722_CRC_POS] != crc8(lt8722_crc8_table, rx_buf, LT8722_CRC_POS,
					   LT8722_CRC_INIT))
		return -EIO;
			   ...
			   struct regulator_init_data *init_data)
{
	...
	ret = device_property_read_u32(&chip->spi->dev, "adi,uv-clamp-microvolt",
				       &chip->uv_clamp_uV);
	if (!ret) {
		/* Interpret the UV clamp value as a negative number */
		chip->uv_clamp_uV = -chip->uv_clamp_uV;

		if (chip->uv_clamp_uV < lt8722_uv_clamp[0] ||
		    chip->uv_clamp_uV > lt8722_uv_clamp[ARRAY_SIZE(lt8722_uv_clamp) - 1])
			return -EINVAL;

		uv_clamp = find_closest(chip->uv_clamp_uV, lt8722_uv_clamp,
					ARRAY_SIZE(lt8722_uv_clamp));

		ret = lt8722_reg_write(chip->spi, LT8722_SPIS_UV_CLAMP, uv_clamp);
		if (ret < 0)
			return ret;
	...
	ret = device_property_read_u32(&chip->spi->dev, "adi,ov-clamp-microvolt",
				       &chip->ov_clamp_uV);
	if (!ret) {
		if (chip->ov_clamp_uV < lt8722_ov_clamp[0] ||
		    chip->ov_clamp_uV > lt8722_ov_clamp[ARRAY_SIZE(lt8722_ov_clamp) - 1])
			return -EINVAL;

		ov_clamp = find_closest(chip->ov_clamp_uV, lt8722_ov_clamp,
					ARRAY_SIZE(lt8722_ov_clamp));

		ret = lt8722_reg_write(chip->spi, LT8722_SPIS_OV_CLAMP, ov_clamp);
		if (ret < 0)
			return ret;
	...
	ret = device_property_read_u32(&chip->spi->dev, "adi,ilimn-microamp",
				       &chip->ilimn_uA);
	if (!ret) {
		/* Interpret the ILIMN value as a negative number */
		chip->ilimn_uA = -chip->ilimn_uA;

		if (chip->ilimn_uA < LT8722_ILIMN_MIN_IOUT ||
		    chip->ilimn_uA > LT8722_ILIMN_MAX_IOUT)
			return -EINVAL;

		ilimn = div_s64(chip->ilimn_uA, -LT8722_ILIM_STEP);

		ret = lt8722_reg_write(chip->spi, LT8722_SPIS_DAC_ILIMN, ilimn);
		if (ret < 0)
			return ret;
	...
	if (!ret) {
		...
		ilimp = div_s64(LT8722_MAX_IOUT - chip->ilimp_uA, LT8722_ILIM_STEP);

		ret = lt8722_reg_write(chip->spi, LT8722_SPIS_DAC_ILIMP, ilimp);
		if (ret < 0)
			return ret;
	...
	ret = device_property_read_u32(&chip->spi->dev, "adi,switch-frequency-hz",
				       &chip->switch_freq_hz);
	if (!ret) {
		for (i = 0; i < ARRAY_SIZE(lt8722_switch_freq); i++)
			if (lt8722_switch_freq[i] == chip->switch_freq_hz)
				break;

		if (i == ARRAY_SIZE(lt8722_switch_freq))
			return -EINVAL;

		ret = lt8722_reg_write_mask(chip->spi, LT8722_SPIS_COMMAND,
					    LT8722_SW_FRQ_SET_MASK, FIELD_PREP(LT8722_SW_FRQ_SET_MASK, i));
		if (ret < 0)
			return ret;
	}

	ret = device_property_read_string(&chip->spi->dev,
					  "adi,switch-frequency-adjust",
					  &chip->switch_freq_adjust);
	if (!ret) {
		for (i = 0; i < ARRAY_SIZE(lt8722_switch_freq_adjust); i++)
			if (strcmp(lt8722_switch_freq_adjust[i], chip->switch_freq_adjust) == 0)
				break;

		if (i == ARRAY_SIZE(lt8722_switch_freq_adjust))
			return -EINVAL;

		ret = lt8722_reg_write_mask(chip->spi, LT8722_SPIS_COMMAND,
					    LT8722_SW_FRQ_ADJ_MASK, FIELD_PREP(LT8722_SW_FRQ_ADJ_MASK, i));
		if (ret < 0)
			return ret;
	}

	ret = device_property_read_string(&chip->spi->dev, "adi,duty-cycle-range",
					  &chip->duty_cycle_range);
	if (!ret) {
		for (i = 0; i < ARRAY_SIZE(lt8722_duty_cycle_range); i++)
			if (strcmp(lt8722_duty_cycle_range[i], chip->duty_cycle_range) == 0)
				break;

		if (i == ARRAY_SIZE(lt8722_duty_cycle_range))
			return -EINVAL;

		ret = lt8722_reg_write_mask(chip->spi, LT8722_SPIS_COMMAND, LT8722_SYS_DC_MASK,
					    FIELD_PREP(LT8722_SYS_DC_MASK, i));
		if (ret < 0)
			return ret;
	}
	...
	ret = device_property_read_u32(&chip->spi->dev, "adi,vcc-vreg-millivolt",
				       &chip->vcc_vreg_mV);
	if (!ret) {
		for (i = 0; i < ARRAY_SIZE(lt8722_vcc_vreg); i++)
			if (lt8722_vcc_vreg[i] == chip->vcc_vreg_mV)
				break;

		if (i == ARRAY_SIZE(lt8722_vcc_vreg))
			return -EINVAL;

		ret = lt8722_reg_write_mask(chip->spi, LT8722_SPIS_COMMAND,
					    LT8722_VCC_VREG_MASK, FIELD_PREP(LT8722_VCC_VREG_MASK, i));
		if (ret < 0)
			return ret;
	}
	...
	if (!ret) {
		for (i = 0; i < ARRAY_SIZE(lt8722_peak_inductor_current); i++)
			if (lt8722_peak_inductor_current[i] == chip->peak_inductor_current_mA)
				break;

		if (i == ARRAY_SIZE(lt8722_peak_inductor_current))
			return -EINVAL;

		ret = lt8722_reg_write_mask(chip->spi, LT8722_SPIS_COMMAND,
					    LT8722_SW_VC_IN_MASK, FIELD_PREP(LT8722_SW_VC_IN_MASK, i));
		if (ret < 0)
			return ret;
	}
	...
	ret = device_property_read_u32(&chip->spi->dev, "adi,power-limit-milliwatt",
				       &chip->power_limit_mW);
	if (!ret) {
		for (i = 0; i < ARRAY_SIZE(lt8722_power_limit); i++)
			if (lt8722_power_limit[i] == chip->power_limit_mW)
				break;

		if (i == ARRAY_SIZE(lt8722_power_limit))
			return -EINVAL;

		ret = lt8722_reg_write_mask(chip->spi, LT8722_SPIS_COMMAND, LT8722_PWR_LIM_MASK,
					    FIELD_PREP(LT8722_PWR_LIM_MASK, i));
		if (ret < 0)
			return ret;
	}
...
static int lt8722_enable(struct regulator_dev *rdev)
{
	...
	return lt8722_reg_write_mask(chip->spi, LT8722_SPIS_COMMAND, LT8722_EN_REQ_MASK,
...
static int lt8722_set_soft_start(struct regulator_dev *rdev)
{
	...
	ret = lt8722_reg_write_mask(chip->spi, LT8722_SPIS_COMMAND,
				    LT8722_SWEN_REQ_MASK, FIELD_PREP(LT8722_SWEN_REQ_MASK, 0x1));
	if (ret < 0)
		return ret;
...
static int lt8722_probe(struct spi_device *spi)
{
	...
	chip->swen_gpio = devm_gpiod_get(&spi->dev, "switch-enable", GPIOD_OUT_LOW);
	if (IS_ERR(chip->swen_gpio))
		return PTR_ERR(chip->swen_gpio);

	/* Reset device */
	ret = lt8722_reg_write_mask(chip->spi, LT8722_SPIS_COMMAND, LT8722_SPI_RST_MASK,
