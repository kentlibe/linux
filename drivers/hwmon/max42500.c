			     ...
			     uint8_t *status)
{
	...
	*status = (uint8_t)FIELD_GET((1 << vm_in), &vm_in_status);
...
static int max42500_read_in(struct device *dev, u32 attr, int channel, long val)
{
	...
	case hwmon_in_label:
		return max42500_get_comp_status(st, channel, channel, val);
	default:
		return -EOPNOTSUPP;
	}
}
...
static const struct hwmon_channel_info *max42500_info[] = {
	HWMON_CHANNEL_INFO(chip,
			   HWMON_C_TEMP_RESET_HISTORY | HWMON_C_IN_RESET_HISTORY | 
			   HWMON_C_CURR_RESET_HISTORY | HWMON_C_POWER_RESET_HISTORY |
