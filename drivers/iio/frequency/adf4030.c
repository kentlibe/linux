				...
				struct adf4030_chan_spec *chan, bool initial)
{
	...
	if (initial) {
		...
		ret = regmap_write(st->regmap, ADF4030_REG(reg + 1),
				   ...
				   FIELD_PREP(ADF4030_LINK_TX_MSK, chan->link_rx) |
	...
	/* EN_DRIVE */
	if (chan->num > 7)
		ret = regmap_set_bits(st->regmap, ADF4030_REG(0x13),
				      BIT(chan->num - 8));
	else
		ret = regmap_set_bits(st->regmap, ADF4030_REG(0x12),
				      BIT(chan->num));
	if (ret)
		return ret;
...
static int adf4030_tdc_measure(const struct adf4030_state *st, u32 channel,
			       ...
			       s64 *result)
{
	...
	int ret, time_diff;
	s64 res_in_fs = 0;
	/* Likely __be32 and needs to be DMA safe */
	u8 vals[3];
	...
	m_time = DIV_ROUND_UP_ULL(1000000ULL * (1 << (ADF4030_CIC_DEC_RATE + 6)),
				  source_out_freq_hz);

	fsleep(m_time);

	/* Wait for TDC finished */
	...
	ret = regmap_bulk_read(st->regmap, ADF4030_REG(0x73), &vals, 3);
	if (ret)
		return ret;
	...
	raw_time_diff = (vals[2] << 16) | (vals[1] << 8) | vals[0];
					    ...
					    u8 iter_number)
{
	...
	if (iter_number == 0)
		return regmap_clear_bits(st->regmap, ADF4030_REG(0x37),
					 ADF4030_EN_ITER_MSK);

	ret = regmap_clear_bits(st->regmap, ADF4030_REG(0x35), ADF4030_ALIGN_THOLD_MSK);
	if (ret)
		return ret;

	ret = regmap_update_bits(st->regmap, ADF4030_REG(0x37),
				 ADF4030_ALIGN_CYCLES_MSK,
				 FIELD_PREP(ADF4030_ALIGN_CYCLES_MSK, iter_number));
	if (ret)
		return ret;

	return regmap_set_bits(st->regmap, ADF4030_REG(0x37),
			       ADF4030_EN_ITER_MSK);
}
					...
					u32 threshold_in_fs)
{
	...
	ret = regmap_set_bits(st->regmap, ADF4030_REG(0x37),
			      ADF4030_EN_CYCS_RED_MSK | ADF4030_EN_ITER_MSK);
	if (ret)
		return ret;
					     ...
					     u32 channel, u32 source_channel)
{
	...
	ret = regmap_update_bits(st->regmap, ADF4030_REG(0x10),
				 ADF4030_TDC_TARGET_MSK,
				 FIELD_PREP(ADF4030_TDC_TARGET_MSK, channel));
	if (ret)
		return ret;

	dev_dbg(&st->spi->dev, "Auto-aligning channel %d to channel %d\n",
...
{
	/* should likely be __be16 and needs to be DMA safe */
	u8 vals[2];
	u32 regval;
	int ret;
	...
	ret = regmap_set_bits(st->regmap, ADF4030_REG(0x72), ADF4030_START_CNV);
	if (ret)
		return ret;
	...
	ret = regmap_bulk_read(st->regmap, ADF4030_REG(0x92), &vals, 2);
	if (ret)
		return ret;

	*die_temp = sign_extend32((vals[1] << 8) | vals[0], 8);
				      ...
				      const char *buf, size_t len)
{
	...
	case BSYNC_ALIGN_EN:
		if (readin < 0 || readin > 1)
			return -EINVAL;
		ret = adf4030_auto_align_single_channel(st, ch->num, ch->reference_chan);
		if (ret)
			return ret;
		return len;
	case BSYNC_ALIGN_THRESH:
...
static struct iio_chan_spec_ext_info adf4030_ext_info[] = {
	...
	{
		.name = "autoalign_enable",
		.read = adf4030_ext_info_read,
		.write = adf4030_ext_info_write,
		.shared = false,
		.private = BSYNC_ALIGN_EN,
	},
	{
			     ...
			     int val, int val2, long mask)
{
	...
	case IIO_CHAN_INFO_FREQUENCY:
		return -ENOTSUPP;
	case IIO_CHAN_INFO_PHASE:
		return adf4030_auto_align_delay(st, ch->num, val);
	}
...
/*
 * The below is called with mask IIO_CHAN_INFO_FREQUENCY which is
 * not implemented in adf4030_write_raw(). Hence this seems useless.
 * REVISIT...
static long adf4030_set_clk_attr(struct clk_hw *hw,
				 long mask,
				 unsigned long val)
{
	...
	return adf4030_write_raw(indio_dev, chan, val, 0, mask);
}
*/
					     ...
					     unsigned long parent_rate)
{
	...
	return st->bsync_freq_odiv_a;
}

static long adf4030_clk_round_rate(struct clk_hw *hw,
				   unsigned long rate,
				   unsigned long *parent_rate)
{
	struct adf4030_output *clkout = adf4030_to_output(hw);
	...
	ret = adf4030_compute_odiv(st->vco_freq, rate, &odiv);
	if (ret)
		return ret;

	return DIV_ROUND_CLOSEST(st->vco_freq, odiv);
}

static int adf4030_clk_set_rate(struct clk_hw *hw,
				unsigned long rate,
				unsigned long parent_rate)
{
	return 0;
...
static const struct clk_ops adf4030_clk_ops = {
	...
	.round_rate = adf4030_clk_round_rate,
	.set_rate = adf4030_clk_set_rate,
};
...
static int adf4030_clk_register(struct adf4030_state *st, unsigned int address)
{
	...
	init.name = st->clk_out_names[address];
...
static int adf4030_parse_fw(struct adf4030_state *st)
{
	...
	/*
	 * I'm assuming we have some range in the possible frequencies.
	 * !FIXME: validate the value in case it's given! This is true
	 * for all other properties.
	 */
	device_property_read_u32(dev, "adi,bsync-secondary-frequency-hz",
				 &st->bsync_freq_odiv_b);

	/* This one is suspicious. Is it really mandatory?! */
	st->bsync_autoalign_ref_chan = ~0;
	ret = device_property_read_u32(dev, "adi,bsync-autoalign-reference-channel",
				       &st->bsync_autoalign_ref_chan);
	if (ret)
		return ret;

	st->bsync_autoalign_iter = 2;
	...
	/* Typically this is not mandatory... REVISIT */
...
static int adf4030_probe(struct spi_device *spi)
{
	...
	if (IS_ERR(st->refin))
		return dev_err_probe(&spi->dev, PTR_ERR(st->refin), "failed to get refin\n");
